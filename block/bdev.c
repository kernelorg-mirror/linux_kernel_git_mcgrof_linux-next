...
static void set_init_blocksize(struct block_device *bdev)
{
	...
	if (order > 0 && folio_order == 0) {
		WARN_ON_ONCE(atomic_read(&bdev->bd_buffer_head_mounted) != 0);
...
static int sb_set_bdev_aops(struct super_block *sb)
{
	...
	if (!(sb->s_type->fs_flags & FS_BUFFER_HEADS)) {
		 ...
		 * nothing to do.
		 */
		if (atomic_read(&bdev->bd_buffer_head_mounted) == 0) {
			if (!uses_buffer_head) {
				sync_blockdev(bdev);
				bdev->bd_inode->i_data.a_ops = &def_blk_aops_iomap;
				kill_bdev(bdev);
		...
		/*
		 * If we don't require iomap it the filesystem may still work
		 * with buffer-heads on the aops but the blocksize limitations
	...
	/*
	 * If a buffer-head filesystem filesystem is about to be mounted
	 * and we already use buffer-heads there is nothing to do.
	 */
	if (uses_buffer_head)
		return 0;
