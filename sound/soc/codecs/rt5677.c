...
static int rt5677_set_dsp_vad(struct snd_soc_component *component, bool on)
{
	...
	if (on && !activity) {
		...
		ret = firmware_request(&rt5677->fw1, RT5677_FIRMWARE1,
			component->dev);
		if (ret == 0) {
			rt5677_spi_write_firmware(0x50000000, rt5677->fw1);
			firmware_release(rt5677->fw1);
		}

		ret = firmware_request(&rt5677->fw2, RT5677_FIRMWARE2,
			component->dev);
		if (ret == 0) {
			rt5677_spi_write_firmware(0x60000000, rt5677->fw2);
			firmware_release(rt5677->fw2);
		}
