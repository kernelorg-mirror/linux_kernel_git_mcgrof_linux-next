			      ...
			      async_cookie_t *async_cookie)
{
	...
	drv_work = kzalloc(sizeof(struct driver_data_file_work), sync_reqs->gfp);
	if (!drv_work)
		return -ENOMEM;

	drv_work->device = device;
	memcpy(&drv_work->params, params, sizeof(struct driver_data_req_params));
...
{
	async_synchronize_cookie_domain(async_cookie+1, &driver_data_async_domain);
}
