 ...
 * for such cases.
 *
 * This looks for the firmare in the built-in kernel. Only if the kernel was
 * built-in with the firmare you are looking for will this return successfully.
 *
 ...
 * @fw: pointer to firmware struct
 * @name: name of firmware file
 * @buf: If set this lets you use a pre-allocated buffer so that the the
 *	built-in firmware into is copied into. This field can be NULL. It is
 *	used by callers such as request_firmware_into_buf() and
 ...
 *
 * This looks for the firmare in the built-in kernel. Only if the kernel was
 * built-in with the firmare you are looking for will this call possibly
