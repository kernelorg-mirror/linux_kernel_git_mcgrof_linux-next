...
file_ra_state_init(struct file_ra_state *ra, struct address_space *mapping)
{
	...
	if (ra->ra_pages < nrpages && nrpages < max_pages)
		ra->ra_pages = nrpages;
	ra->prev_pos = -1;
}
		...
		unsigned long nr_to_read)
{
	...
	unsigned int nr_of_pages = (1UL << folio_order);
		...
		struct folio *folio, unsigned long req_size)
{
	...
	if (index == expected || index == (ra->start + ra->size)) {
		ra->start += ra->size;
		ra->size = get_next_ra_size(ra, nrpages, max_pages);
...
initial_readahead:
	ra->start = index;
	ra->size = get_init_ra_size(req_size, nrpages, max_pages);
...
readit:
	...
	if (index == ra->start && ra->size == ra->async_size) {
		add_pages = get_next_ra_size(ra, nrpages, max_pages);
