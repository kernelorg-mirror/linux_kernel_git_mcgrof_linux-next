 ...
 * specific. Linux has historically implicitly used linker tables for ages,
 * however they were all built in an adhoc manner which requires linker script
 * modifications, per architecture. The linker table API provides a general
 * linker table facility so that new linker tables can be introduced and
 * supported on Linux by only changing C code.
 *
 * Linker tables help you group together related data and code in an efficient
 * way. Linker tables can be used to help simplify init init sequence, they
 ...
 * contrib "[0]).  To see how this code evolved or to extend and test and use
 * this code in userspace refer to the userspace linker-table tree [1].  This
 * repository can be used for ease of testing of extensions and sampling of
 * changes prior to inclusion into Linux, it is intended to be kept up to date
 * to match Linux's solution. Contrary to iPXE's solution, which strives to
 * force compilation of everything using linker tables, Linux's solution allows
 * for developers to be selective over where one wishes to force compilation,
 * this then is just an optional feature for the Linux linker table solution.
 *
