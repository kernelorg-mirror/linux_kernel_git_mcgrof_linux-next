 ...
 * module created. The sysfs store / read file operations are gauranteed to
 * exist through the use of kernfs's active reference (see kernfs_active()).
 * If a sysfs file operation is being run, the module which created it must
 * still exist as the module is in charge of removing the same sysfs file being
 * read. Also, a sysfs / kernfs file removal cannot happen unless unless the
 * same file is not active.
 *
