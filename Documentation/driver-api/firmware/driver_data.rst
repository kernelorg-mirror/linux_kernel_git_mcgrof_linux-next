...
Users of firmware request APIs has grown now to include users which are not
looking for "firmware", but instead general driver data files which has been
decided to be kept oustide of the kernel. The driver data APIs addresses
rebranding of firmware as generic driver data files, and provides a flexible
API which mitigates collateral evolutions on the kernel when new functionality
...
request parameter data structure. This data structure can grow with new
...
this requirement is part of the request parameter data structure it also allows
...
The driver data API relies on async cookies to enable users to synchronizing
