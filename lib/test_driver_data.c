...
static
struct driver_data_test_device *misc_dev_to_test_dev(struct miscdevice *misc_dev)
{
...
static int __driver_data_config_init(struct test_config *config)
{
	...
	ret = config_copy_name(config, TEST_DRIVER_DATA,
			       strlen(TEST_DRIVER_DATA));
	if (ret < 0)
		goto out;

	ret = config_copy_default_name(config, TEST_DRIVER_DATA,
				       strlen(TEST_DRIVER_DATA));
	if (ret < 0)
		goto out;
...
static ssize_t test_dev_config_show_bool(struct driver_data_test_device *test_dev,
					 char *buf,
					 bool config)
{
	bool val;
...
static ssize_t test_dev_config_show_int(struct driver_data_test_device *test_dev,
					char *buf,
					int config)
{
	int val;
...
struct driver_data_test_device *alloc_test_dev_driver_data(int idx)
{
	...
	if (ret < 0) {
		pr_err("Cannot alloc driver_data_config_init()\n");
		goto err_out_free;
	}
