 ...
 * as some helpers documented in this chapter.
 */
 ...
 * CONFIG_DEBUG_RODATA and implement a mark_rodata_ro() call. mark_rodata_ro()
 * can be used for instance to mark specific sections as read-only or
 * non-executable.
 *
 ...
 * DOC: .rodata
 *
 * ELF section used data which must be protected from write access.
 */
 ...
 * DOC: .initcall
 *
 * ELF section used for subsystem init calls. These calls are
 ...
 * each section entry addd. For instance if a linker script uses:
 *
 *    SORT(.foo.*)
 *
