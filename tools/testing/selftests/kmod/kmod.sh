...
usage()
{
	...
	echo Example uses:
	echo
	echo "kmod.sh		-- executes all tests"
	echo "kmod.sh -t 0008	-- Executes test ID 0008 number of times is recomended"
	echo "kmod.sh -w 0008	-- Watch test ID 0008 run until an error occurs"
	echo "kmod.sh -s 0008	-- Run test ID 0008 once"
	echo "kmod.sh -c 0008 3	-- Run test ID 0008 three times"
	echo
	list_tests
	exit 1
}
...
function test_case()
{
	...
	while [ $i -lt $NUM_TESTS ]; do
		test_num $1
		watch_log $i kmod_test_$1 noclear
		RUN_TEST=kmod_test_$1
		$RUN_TEST
		let i=$i+1
	done
}
