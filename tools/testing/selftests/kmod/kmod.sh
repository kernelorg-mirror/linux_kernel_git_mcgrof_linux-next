...
check_umh()
{
	...
	if [[ "$RC" == "256" ]]; then
		...
		echo "check_umh: you have and old umh which didn't wrap errors with WEXITSTATUS(). This is OK!"
	elif [[ "$RC" != "1" ]]; then
		echo "check_umh: Unexpected return value with no modprobe argument: $RC"
		exit
	else
		echo "check_umh: You have a new umh which wraps erros with WEXITSTATUS(). This is OK!"
	fi
}
